...
def find_best_road(edges, start=0, road_so_far=(), score_so_far=(0, 0)):
    ...
    for node, count in edges[start].items():
        ...
        if road_so_far.count(edge) < count:
            ...
            extended_score = (score_so_far[0] + 1, score_so_far[1] + sum(edge))
...
assert find_best_road(parse_input(sample_input)) == ((frozenset([0, 1]),
                                                      frozenset([1, 10]),
                                                      frozenset([10, 9])), 31)

with open('puzzle-input.txt') as f:
    path, score = find_best_road(parse_input(f))
    print(score)
