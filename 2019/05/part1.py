...
*   It is important to remember that the instruction pointer should increase by
    the number of values in the instruction after the instruction
...
test was from the expected value, where 0 means the test was
...
def compute(memory):
    ...
    try:
        ...
        assert exc.value is memory
        return memory
    else:
        raise ValueError("test fails: unexpected output")
...
def run_diagnostic_program(memory, input_values):
    output_values = list(vm(memory, input_values))
    assert all(x == 0 for x in output_values[:-1])
