...
def maxlen(re_ast):
    ...
    return tree_height(graph, (0, 0))

assert maxlen(parse_re('^WNE$')) == 3
assert maxlen(parse_re('^ENWWW(NEEE|SSE(EE|N))$')) == 10
assert maxlen(parse_re('^ENNWSWW(NEWS|)SSSEEN(WNSE|)EE(SWEN|)NNN$')) == 18
assert maxlen(parse_re('^ESSWWN(E|NNENN(EESS(WNSE|)SSS|WWWSSSSE(SW|NNNE)))$')) == 23
assert maxlen(parse_re('^WSSEESWWWNW(S|NENNEEEENN(ESSSSW(NWSW|SSEN)|WSWWN(E|WWS(E|SS))))$')) == 31

with open('puzzle-input.txt') as f:
    regex = f.read().strip()
print(maxlen(parse_re(regex)))
