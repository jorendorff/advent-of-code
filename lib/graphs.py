...
def _state_to_path(state):
    rpath = [state.node]
    while state.prev_edge is not None:
...
def breadth_first_weighted_paths(start_set, destination_test, ways, key=id):
    ...
    while queue:
        ...
        if destination_test(state.node):
            yield _state_to_path(state)

        for edge, weight, after_node in ways(node):
            k = key(after_node)
            after_state = State(
...
def shortest_weighted_path(start_set, destination_test, ways, key=id):
    for path in breadth_first_weighted_paths(start_set, destination_test, ways, key):
        return path
    else:
        return None
