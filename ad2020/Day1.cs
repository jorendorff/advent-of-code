// Dafny program Day1.dfy compiled into C#
// To recompile, use 'csc' with
//     /r:System.Numerics.dll /r:System.Collections.Immutable.dll /r:System.Runtime.dll
// You'll find a copy of System.Collections.Immutable.dll in the Dafny distribution, and
// Dafny will have tried to copy this file into the target directory if you compiled to
// an executable (with a Main method). The csc compiler should know where to pick up the
// other two DLLs.
// You should also choose either
//     /target:exe
// or
//     /target:library
...
// Dafny 3.0.0.20820
// Command Line Options: Day1.dfy fileionative.cs
// Day1.dfy

datatype Option<T> = None | Some(value: T)

method FindEntries(expenses: array<nat>) returns (result: Option<(nat, nat)>)
  ensures if result.Some? then exists i: int, j: int :: 0 <= i < j < expenses.Length && result.value.0 == expenses[i] && result.value.1 == expenses[j] && expenses[i] + expenses[j] == 2020 else !exists i: int, j: int :: 0 <= i < j < expenses.Length && expenses[i] + expenses[j] == 2020
  decreases expenses
{
  var seen := new bool[2021] ((i: int) => false);
  assert forall i: int :: 0 <= i < seen.Length ==> seen[i] == false;
  var c := 0;
  while c < expenses.Length
    invariant 0 <= c <= expenses.Length
    invariant forall v: int :: 0 <= v <= 2020 ==> (seen[v] <==> v in expenses[..c])
    invariant !exists i: int, j: int :: 0 <= i < j < c && expenses[i] + expenses[j] == 2020
    decreases expenses.Length - c
  {
    var val := expenses[c];
    if 0 <= val <= 2020 {
      var other := 2020 - val;
...
method Main()
{
  var expenses := FileIO.ReadFileOfNumbers(""data.txt"");
...
class FileIO {
  static method ReadFileOfNumbers(name: string) returns (numbers: array<nat>)
    decreases name
  {
    var nameChars := ToArray(name);
    var nums32 := FileIO.ReadFileOfNumbersNative(nameChars);
    numbers := new nat[nums32.Length];
    var i := 0;
    while i < nums32.Length
      invariant 0 <= i <= nums32.Length
      decreases nums32.Length - i
    {
      numbers[i] := nums32[i] as nat;
      i := i + 1;
    }
  }

  static method {:axiom} ReadFileOfNumbersNative(name: array<char>) returns (numbers: array<nat32>)
...
namespace _module {
  ...
  public partial class __default {
    public static Option<_System.Tuple2<BigInteger,BigInteger>> FindEntries(BigInteger[] expenses)
    {
      Option<_System.Tuple2<BigInteger,BigInteger>> result = Option<_System.Tuple2<BigInteger,BigInteger>>.Default;
      bool[] _40_seen;
      bool[] _nw0 = new bool[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(new BigInteger(2021), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      Func<BigInteger,bool> _arrayinit0 = Dafny.Helpers.Id<Func<Func<BigInteger,bool>>>(() => ((System.Func<BigInteger, bool>)((_41_i) => {
        return false;
      })))();
      ...
      _40_seen = _nw0;
      BigInteger _42_c;
      _42_c = BigInteger.Zero;
      while ((_42_c) < (new BigInteger((expenses).Length))) {
        BigInteger _43_val;
        _43_val = (expenses)[(int)(_42_c)];
        if (((_43_val).Sign != -1) && ((_43_val) <= (new BigInteger(2020)))) {
          BigInteger _44_other;
          _44_other = (new BigInteger(2020)) - (_43_val);
          if ((_40_seen)[(int)(_44_other)]) {
            result = @Option<_System.Tuple2<BigInteger,BigInteger>>.create_Some(@_System.Tuple2<BigInteger,BigInteger>.create(_44_other, _43_val));
            return result;
          }
          (_40_seen)[(int)((_43_val))] = true;
        }
        _42_c = (_42_c) + (BigInteger.One);
      }
      result = @Option<_System.Tuple2<BigInteger,BigInteger>>.create_None();
      return result;
      return result;
    }
    public static void _Main()
    {
      BigInteger[] _45_expenses;
      BigInteger[] _out0;
      _out0 = FileIO.ReadFileOfNumbers(Dafny.Sequence<char>.FromString("data.txt"));
      _45_expenses = _out0;
      Option<_System.Tuple2<BigInteger,BigInteger>> _46_result;
      Option<_System.Tuple2<BigInteger,BigInteger>> _out1;
      _out1 = __default.FindEntries(_45_expenses);
      _46_result = _out1;
      Dafny.Helpers.Print(_46_result);
    }
    public static __A[] ToArray<__A>(Dafny.ISequence<__A> s)
    {
      __A[] a = new __A[0];
      __A[] _nw1 = new __A[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(new BigInteger((s).Count), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      Func<BigInteger,__A> _arrayinit1 = Dafny.Helpers.Id<Func<Dafny.ISequence<__A>,Func<BigInteger,__A>>>((_47_s) => ((System.Func<BigInteger, __A>)((_48_i) => {
        return (_47_s).Select(_48_i);
      })))(s);
      for (var _arrayinit_01 = 0; _arrayinit_01 < new BigInteger(_nw1.Length); _arrayinit_01++) {
  ...
  public partial class FileIO {
    public static BigInteger[] ReadFileOfNumbers(Dafny.ISequence<char> name)
    {
      BigInteger[] numbers = new BigInteger[0];
      char[] _49_nameChars;
      char[] _out2;
      _out2 = __default.ToArray<char>(name);
      _49_nameChars = _out2;
      int[] _50_nums32;
      int[] _out3;
      _out3 = FileIO.ReadFileOfNumbersNative(_49_nameChars);
      _50_nums32 = _out3;
      BigInteger[] _nw2 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(new BigInteger((_50_nums32).Length), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      numbers = _nw2;
      BigInteger _51_i;
      _51_i = BigInteger.Zero;
      while ((_51_i) < (new BigInteger((_50_nums32).Length))) {
        (numbers)[(int)((_51_i))] = new BigInteger((_50_nums32)[(int)(_51_i)]);
