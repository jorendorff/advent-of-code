...
def main (args : List String) : IO Unit := do
  let [filename] := args | IO.println "usage: day01 inputs/example.txt"
  let inputStr <- IO.FS.readFile filename
  let result := parser ⟨inputStr, inputStr.startValidPos⟩
  match result with
  | ParseResult.error pos err  => do
    IO.println s!"parse error at {pos.snd.offset}: {err}"
