...
def parse_reactions(text):
    def parse_term(term):
        n, c = term.strip().split()
        return int(n), c

    reactions = {}
    for line in text.splitlines():
        in_side, out_side = line.strip().split(" => ")
        out_n, out_c = parse_term(out_side)
        assert out_c not in reactions
        reactions[out_c] = out_n, [parse_term(term) for term in in_side.split(",")]
    return reactions
    ...
    while True:
        ...
        for chemical, amount in budget.items():
            if chemical != 'ORE' and amount < 0:
                ...
                output_amount, reagents = reactions[chemical]
                recipes_needed = round_up(-amount, output_amount)
                for in_amount, in_chemical in reagents:
                    budget[in_chemical] -= in_amount * recipes_needed
