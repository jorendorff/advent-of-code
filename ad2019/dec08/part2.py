...
def render(img):
    picture = [list(row) for row in img.layers[0]]
    ...
    pic = ''
    for row in picture:
        s = ['#' if c == '0' else ' ' if c == '1' else c
             for c in row]
        pic += ''.join(s) + '\n'
    return pic


assert render(part1.SpaceImage.from_text(2, 2, '0222112222120000')) == '# \n #\n'


def main():
    img = part1.SpaceImage.load("puzzle-input.txt", part1.WIDTH, part1.HEIGHT)
    print(render(img))
