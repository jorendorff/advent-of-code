...
POINT_RE = re.compile(r'^<x=(0|-?[1-9][0-9]*), *y=(0|-?[1-9][0-9]*), *z=(0|-?[1-9][0-9]*)>$')
...
def tick(state):
    ...
    for a in state:
        for b in state:
            if a is not b:
                a.attract(b)  # b will attract a separately
...
def main():
    moons = parse_scenario(puzzle_input())
    for _ in range(1000):
        tick(moons)
    print(system_total_energy(moons))
